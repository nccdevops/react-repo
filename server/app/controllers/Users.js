...
const User = {
   ...
   */
  logout(req, res) {
    db.Users.findById(req.tokenDecode.userId).then((user) => {
      user.update({ active: false }).then(() =>
        res.status(200).send({
  ...
  getAll(req, res) {
    ...
    db.Users.findAndCountAll(req.dmsFilter).then((users) => {
      if (users) {
        const condition = {
          count: users.count,
  ...
  findUserDocuments(req, res) {
    ...
    db.Users.findById(req.params.id).then((user) => {
      ...
      db.Documents.findAndCountAll(req.dmsFilter).then((docs) => {
        const condition = {
          count: docs.count,
          limit: req.dmsFilter.limit,
          offset: req.dmsFilter.offset,
